...
$sql = "SELECT * FROM transaction_requst  WHERE status = 'processing' ";
...
while ($row = $result->fetch_assoc()) {
    ...
    echo "<td class='hedlines green1'><a class='acclink' href='recharge_request.php?id=" . $row["user_id"] . "&transaction_id=" . $row["transaction_id"] . "&amount=" . $row["camount"] . "&status=accepted'>Accept</a>  <a class='rejlink' href='recharge_request.php?id=" . $row["user_id"] . "&transaction_id=" . $row["transaction_id"] . "&amount=" . $row["camount"] . "&status=rejected'>Reject</a></td>";
}
