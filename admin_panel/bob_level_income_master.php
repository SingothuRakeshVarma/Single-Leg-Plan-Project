...
if (isset($_POST['submit'])) {
  ...
  $query = "SELECT * FROM bob_level_income WHERE type ='$packageorproduct' AND packagename = '$packagename'";
  $result = mysqli_query($con, $query);
  if (mysqli_num_rows($result) > 0) {
    echo '<script>alert("ROB Level Income data Already Exit!");window.location.href = "bob_level_income_master.php";</script>';
  } else {
    ...
    if (empty($user_id) || empty($packageorproduct) || empty($category) || empty($sub_category) || empty($packagename) || empty($self) || empty($level_1) || empty($level_2) || empty($level_3) || empty($level_4) || empty($level_5)) {
      ...
      $sql = " INSERT INTO `bob_level_income`(`bob_id`, `type`, `category`, `sub_category`, `packagename`, `self`, `level_1`, `level_2`, `level_3`, `level_4`, `level_5`) 
    VALUES ('$user_id','$packageorproduct','$category','$sub_category','$packagename','$self','$level_1','$level_2','$level_3','$level_4','$level_5')";

      if ($con->query($sql) === TRUE) {
        echo '<script>alert("BOB Level Income data saved successfully!");window.location.href = "bob_level_income_master.php";</script>';
      } else {
