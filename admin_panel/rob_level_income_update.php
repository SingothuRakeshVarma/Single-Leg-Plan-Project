...
if (isset($_GET['rob_id'])) {
  ...
  $query = "SELECT * FROM `rob_level_income` WHERE rob_id = ?";
...
if (isset($_POST['submit'])) {
  ...
  $updateQuery = "UPDATE `rob_level_income` 
                   SET type = ?, 
                       category = ?, 
                       sub_category = ?, 
                       packagename = ?, 
  ...
  if ($stmt->execute()) {
    echo '<script>alert("ROB Level Income data Update successfully!");window.location.href = "rob_level_table.php";</script>';
  } else {
...
<section>
  ...
  <div class="page-container">
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" class="check-details" method="post">
      <input type="hidden" name="rob_id" value="<?php echo $row['rob_id']; ?>">
